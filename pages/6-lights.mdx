...
First, let's make a light emitting material. We need to add an `emitted` function. Like the
background, it just tells the ray what color it is and performs no reflection. It's very simple:
...
provides a default implementation that returns black:
...
do this, we'll add a `background` color to the camera and pay attention to the new
`color_from_emission` value.
...
Since we're removing the code that determined the color of the sky when a ray missed all objects,
we pass in a new color value for our old scene renders — a flat bluish-white for the whole sky.
...
Fool around with making some sphere lights too:
