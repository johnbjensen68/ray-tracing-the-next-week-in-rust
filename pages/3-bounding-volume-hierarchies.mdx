...
<Tabs>
  <Tab label="Rust">
    ```rust filename="interval.rs | interval::expand() method"
    impl Interval {
...
Every primitive needs to be able to report its bounding box, so the BVH can group them into a hierarchy of boxes. We add a `bounding_box` method to the `Hittable` trait. Default-constructed intervals (and therefore default-constructed AABBs) are empty by design, so an empty `hittable_list` gives back an empty box and the math still works out. Animated objects should return bounds that cover their entire range of motion from time=0 to time=1.
...
This needs an `aabb` constructor that takes two boxes as input. We'll first add a new interval constructor that does the same thing for intervals:
...
Then use it to construct an AABB from two input boxes:
...
The trickiest part of any spatial efficiency structure is building it, and the BVH is no exception — that work happens in the constructor. The hit function will work as long as we divide each node's objects into two sub-lists; tighter divisions just make traversal faster, not more correct. We'll go with a simple recipe at each node:
...
<Tabs>
  <Tab label="Rust">
    ```rust filename="rtweekend.rs | A function to return random integers in a range"
...
<Tabs>
  <Tab label="Rust">
    ```rust filename="bvh.rs | BvhNode constructor using longest axis"
    impl BvhNode {
        pub fn new(objects: &mut Vec<Arc<dyn Hittable>>, start: usize, end: usize) -> Self {
...
<Tabs>
  <Tab label="Rust">
    ```rust filename="aabb.rs | New aabb constants and longest_axis() function"
    impl Aabb {
...
The output is again identical to the non-BVH render, but a little faster — the original book reports about 18% on top of the random-axis BVH.
