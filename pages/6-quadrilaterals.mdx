...
<Tabs>
  <Tab label="Rust">
    ```rust filename="aabb.rs | New Aabb::pad_to_minimums() method"
    impl Aabb {
...
<Tabs>
  <Tab label="Rust">
    ```rust filename="quad.rs | Caching the planar values"
    pub struct Quad {
        q: Point3,
        u: Vec3,
        v: Vec3,
...
<Tabs>
  <Tab label="Rust">
    ```rust filename="quad.rs | Caching the quadrilateral's w value"
    pub struct Quad {
        q: Point3,
        u: Vec3,
        v: Vec3,
...
<Tabs>
  <Tab label="Rust">
    ```rust filename="quad.rs | Final quad class"
    impl Hittable for Quad {
        fn hit(&self, r: &Ray, t_min: f64, t_max: f64) -> Option<HitRecord> {
            let ray_t = Interval::new(t_min, t_max);
            let denom = vec3::dot(self.normal, r.direction());
...
<Tabs>
  <Tab label="Rust">
    ```rust filename="main.rs | A new scene with quads"
    fn quads() {
