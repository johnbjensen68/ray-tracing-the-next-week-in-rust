...
<Tabs>
  <Tab label="Rust">
    ```rust {2,7,10-12,25} filename="material.rs | Lambertian material with texture"
...
<Tabs>
  <Tab label="Rust">
    ```rust {4-8,12} filename="main.rs | Checkered texture in use"
...
```rust {2-5,17-19,31-34} filename="camera.rs | Camera struct and constructor with image and sampling settings"
pub struct Camera {
    aspect_ratio: f64,
    image_width: usize,
...
<Tabs>
  <Tab label="Rust">
    ```rust {9-10,14-16} filename="sphere.rs | Sphere UV coordinates from hit"
...
<Tabs>
  <Tab label="Rust">
    ```rust {30,33} filename="main.rs | Rendering with a texture map of Earth"
    fn earth() {
