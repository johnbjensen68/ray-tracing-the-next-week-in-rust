...
## Moving the Render Loop into Camera

The scene functions above call `cam.render(&world)`, but our Rust port has so far kept the rendering loop and the `ray_color` helper in `main.rs`. The C++ version of this book moved them onto the `camera` class back in *Ray Tracing in One Weekend*; we're catching up now so that each scene function only has to configure a `Camera` and ask it to render.

First, extend `Camera` with the image-size and sampling settings that the loop needs, and add `image_width`, `samples_per_pixel`, and `max_depth` to the constructor:

```rust filename="camera.rs | Camera struct and constructor with image and sampling settings"
pub struct Camera {
    aspect_ratio: f64,
    image_width: usize,
...
Move `ray_color` from `main.rs` onto `Camera` as a private method, and add a `render` method that drives the per-pixel sampling loop:

```rust filename="camera.rs | ray_color and render methods on Camera"
impl Camera {
    // ...
...
With the loop on `Camera`, `main.rs` becomes a simple dispatcher and each scene function configures its own camera and calls `cam.render(&world)`:

```rust filename="main.rs | main reduced to scene dispatch"
fn bouncing_spheres() {
...
The free `ray_color` function and the per-pixel loop that previously lived in `main()` are gone — every scene from here on simply hands its `Camera` and `World` to `cam.render`.
