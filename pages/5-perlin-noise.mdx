...
<Tabs>
  <Tab label="Rust">
    ```rust {34,38} filename="main.rs | Scene with two Perlin-textured spheres"
    fn perlin_spheres() {
...
<Tabs>
  <Tab label="Rust">
    ```rust {3,7-8,14} filename="texture.rs | NoiseTexture with scale"
    pub struct NoiseTexture {
        noise: Perlin,
        scale: f64,
    }
...
<Tabs>
  <Tab label="Rust">
    ```rust {4} filename="main.rs | Perlin-textured spheres with a scale to the noise"
    fn perlin_spheres() {
        let mut world = HittableList::new();
...
The Rust port also needs a small helper on `Vec3` to seed the lattice array. C++ can write `vec3 c[2][2][2];` and the elements default-construct to the origin, but Rust's fixed-size array initializer needs an explicit value, so we add `Vec3::zero()`:

```rust filename="vec3.rs | Vec3::zero helper for array initialization"
impl Vec3 {
    pub fn zero() -> Vec3 {
        Vec3 { e: [0.0, 0.0, 0.0] }
    }
...
<Tabs>
  <Tab label="Rust">
    ```rust {3} filename="texture.rs | NoiseTexture with range remapping"
    impl Texture for NoiseTexture {
        fn value(&self, _u: f64, _v: f64, p: &Point3) -> Color {
...
<Tabs>
  <Tab label="Rust">
    ```rust {3} filename="texture.rs | NoiseTexture using turbulence"
    impl Texture for NoiseTexture {
        fn value(&self, _u: f64, _v: f64, p: &Point3) -> Color {
...
<Tabs>
  <Tab label="Rust">
    ```rust {2-3} filename="texture.rs | Marble-like noise texture"
    fn value(&self, _u: f64, _v: f64, p: &Point3) -> Color {
